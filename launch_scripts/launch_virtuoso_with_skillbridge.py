...
def main(args):
    ...
    for tid in range(n_virtuosos):
        ...
        with open(f'./skill_launch_files/start{tid}.il', 'w') as f:
            ...
            f.write('load("../openpdk_libs/Generators/CCSinvokeCdfCallbacks.il")\n')

        if n_virtuosos_gui > 0:
            subprocess.run(f'gnome-terminal -- bash -c "cd ~/cadence/pdk/openpdk; source toolsenv; virtuoso -restore ../openpdk_libs/Generators/skill_launch_files/start{tid}.il"', shell=True)
            n_virtuosos_gui -= 1
        elif n_virtuosos_hidden > 0:
            subprocess.run(f'gnome-terminal -- bash -c "cd ~/cadence/pdk/openpdk; source toolsenv; virtuoso -restore -nograph ../openpdk_libs/Generators/skill_launch_files/start{tid}.il"', shell=True, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
